...
TODO: Get rid of the variable "C" and replace it with $(\pi,s)$

Our image translation module, $T$, is based on the image translation module used in \citep{surgical_video_translation}, which is a based on a variant \cite{surgical_image_translation} of MUNIT \cite{munit}. The main differences between our image translation model and MUNIT is that our style code is fixed making it unimodal, and noise is injected into the latent codes during training to prevent overfitting. During inference however, this intermediate noise is removed and our image translation module is completely determinstic. In addition, we use a different reconstruction criterion for cycle consistency. Apart from these differences, our image translation module has the same loss formulations and generator/encoder/discriminator network architectures as MUNIT. 
...
In our translation module, we define
$T_{A\rightarrow B}\left(c\right) = G_B\left(E_A\left(c\right)\right)$
and 
$T_{B\rightarrow A}\left(p\right) = G_A\left(E_B\left(p\right)\right)$
where $E_A, E_B, G_A, G_B$ are encoders and generators for domains A and B respectively, $p \in B$ is a batch of real photos, and $c\in A$ is a batch of simulated uvl+texture projection $(s, \pi)$ concatenations. 
We also define the outputs of these translators as fake photos $\hat{p} = T_{A\rightarrow B}(c)$ and fake projection/uvl scenes $(\hat{\pi},\hat{s}) = \hat{c} = T_{A\rightarrow B}(c)$.
...
To ensure cycle consistency, we impose reconstruction losses to ensure
% $c \approx G_A(E_B(G_B(E_A(c)))) = T_{B\rightarrow A}(T_{A\rightarrow B}(c)) = T_{B\rightarrow A}(\hat{p})$
$c \approx T_{B\rightarrow A}(\hat{p})$
and vice versa
% $p \approx G_B(E_A(G_A(E_B(p)))) = T_{A\rightarrow B}(T_{B\rightarrow A}(c)) = T_{A\rightarrow B}(\hat{c}) = T_{A\rightarrow B}(\hat{\pi},\hat{s})$. 
...
We also impose reconstruction losses to ensure that $G_A(E_A(c)) \approx c$ and $G_B(E_B(p)) \approx p$, which essentially acts as an autoencoder loss. 
% We also have optional reconstruction losses to ensure 

Our reconstruction loss uses a combination of L2 loss and multi scale structural image similarity (MS-SSIM), introduced in \citep{msssim}. For our uses, MS-SSIM is a metric that measures the similarity of two images on a scale from 0 to 1. Our reconstruction loss is $\ell_{recon}(x,y) = -msssim(x,y) + \mu\left((x-y)^2\right)$.

We also have discriminators $D_A$ and $D_B$ 
