...
\label{sec:data}
	...
	TRITON's goal is to turn 3d simulated images into realistic fake photographs without any matching pairs, while maintaining high surface consistency.
	It does this by simultaneously learning both an image translator and a set of realistic textures.
	These translations can be useful for downstream tasks, especially robotic policy learning from camera data.
	...
	We represent each 3d state with a simple $(r,g,b)$ image, in a simple enough format that any decent robotic simulator should able to provide.
...
\label{sec:method}
	...
	\subsection{Neural Neural Texture Projection}
		...
		For each texture $\tau_i \in \tau\toL$, 
		\begin{equation}
			\tau_i: (u,v) \in\real^2 \rightarrow (r,g,b) \in\real^3 % \eqand 
		\end{equation}
		
		Given scenes $s$, we obtain projections $\pi$ by applying a texture $\tau_i \in \tau\toL$ to every pixel $(u,v,l) \in s$ individually, where texture index $i=I(l)$ is decided by the label value $l$ of that scene pixel.
		...
		\begin{equation}
			...
			% \pi_{n,c,x,y}=\tau_i(s_{n,1,x,y},s_{n,2,x,y}) \eqwhere 1\le n\le N,\ \  1\le c\le 3 ,\ \ 1\le x\le W,\ \ 1\le y\le H,\ \ \text{and} \ \ i=I(s_{n,3,x,y})
			\pi_{n,c,x,y}=\tau_i(s_{n,1,x,y},s_{n,2,x,y})
		\end{equation}
		where $1\le n\le N,\ \ 1\le c\le 3 ,\ \ 1\le x\le W,\ \ 1\le y\le H,\ \ \text{and} \ \ i=I(s_{n,3,x,y})$.
		% where $n\in 1\dots N,\ \ c\in 1\dots 3 ,\ \ 1\le x\le W,\ \ 1\le y\le H,\ \ \text{and} \ \ i=I(s_{n,3,x,y})$.
