...
To do this we add an image similarity loss that makes $\pi$ look like its translation $\hat{p}$. 
...
Up until now we have not put any constraints on this neural texture. With only the above constraints, $\tau$ might look wildly different each time you train it.
As seen in the top row of figure \ref{fig:texture_realism_ablation}, the neural texture might not look anything like the translation results. 

We introduce a new loss, called texture realism loss. $\lTR$ image similarity loss that encourages $\pi \approx \ph$.
...
This image similarity loss is calculated the same way image similarity losses are calculated in section \ref{sec:image_translation}: L2 loss minus MSSSIM.
