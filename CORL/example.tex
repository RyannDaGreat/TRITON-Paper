...
\begin{figure}[H]
	...
	\caption{
		...
		%Always use function notation tau(u,v) instead of just tau (in figure as well) 
...
\subsection{Neural Neural Texture and Projection}
	...
	\begin{equation}
		\qquad \tau: (u,v) \in\real^2 \rightarrow (r,g,b) \in\real^3 % \eqand 
	\end{equation}
	
	Given a 3d UVL scene $s$, we obtain texture projection $\pi=\tau\toL(s)$ by applying $\tau\toL$ to every pixel $(u,v,l) \in s$ individually.
	Each pixel in the resulting projection $(r,g,b)\in\pi$ is the result of selecting a texture index $i$ based on the label value $l$, then applying $\tau_i$ to the $u,v$ values.
	\begin{equation}
		\pi=\tau\toL(s) \eqand (r,g,b) \in \pi = \tau_i(u,v)
	\end{equation}
	...
	% TRITON's neural neural texture module lets it create features on the surfaces of objects, and thus take advantage of 3d geometric priors. 
			...
			% The neural texture applied to a particular scene pixel $(u,v,l) \in s$ is multiplexed by $l$, and the respective output pixel $(r,g,b) \in \pi = \tau_l(u,v)$. 
			...
			% When applied to a UVL scene, it is applied to each $(u,v,l) \in s$ pixel individually where every pixel $(r,g,b) \in \pi = \tau$ 
			...
			% 			Neural Neural Textures $\tau\toL(u,v)$: Given an input scene $s$ depicting 3d objects with labels $1\dots L$, we apply a function on their surface's UV values with neural textures $\tau\toL$ to obtain projection $\pi$.
	...
	In practice, we found that TRITON learns faster with fourier-feature textures than raster textures, as well as being smoother and less noisy. 
...
\label{sec:unprojection_consistency_loss}
	...
	Unprojection Consistency Loss $\lUC$: To keep the object surfaces consistent between a batch of $N$ scenes, we impose a pixel-wise similarity loss between these surfaces by warping them back into a common texture space using the UV and label values contained in the original scene. 
	\begin{equation}
		u\toNL=\text{unproject}(\ph\toN,s\toN)\ \ \ \text{ and } \ \ \ \lTR \Rightarrow u_1 \approx u_2 \dots \approx u_N
	\end{equation}


	To maintain surface consistency, we sample a random batch of UVL scenes $S\toN$, and translate them into $\ph\toN$.
